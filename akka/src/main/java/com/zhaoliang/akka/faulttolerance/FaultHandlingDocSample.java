package com.zhaoliang.akka.faulttolerance;
...
public class FaultHandlingDocSample {
			...
			if (msg.equals(WorkerApi.Start) && progressListener == null) {
				...
				getContext()
						...
						.schedule(Duration.Zero(),
								  Duration.create(1, "second"), getSelf(), WorkerApi.Do,
						...
						ask(counterService, CounterServiceApi.GetCurrentCount, askTimeout).mapTo(
								...
								new Mapper<CounterServiceApi.CurrentCount, WorkerApi.Progress>() {
									public WorkerApi.Progress apply(CounterServiceApi.CurrentCount c) {
										return new WorkerApi.Progress(100.0 * c.count
												/ totalCount);
									}
								}, getContext().dispatcher()),
						getContext().dispatcher()).to(progressListener);
			} else {
				unhandled(msg);
			}
		}
	 ...
	 * Replies with CurrentCount when it is asked for CurrentCount.
	...
	public static class CounterService extends UntypedActor {
		...
		/**
		 * The child storage is restarted in case of failure, but after 3
		 * restarts, and still failing it will be stopped. Better to back-off
		 * than continuously failing. When it has been stopped we will schedule
		 * a Reconnect after a delay. Watch the child so we receive Terminated
		 * message when it has been terminated.
		 */
		void initStorage() {
			storage = getContext().watch(
					getContext()
							.actorOf(Props.create(Storage.class), "storage"));
		...
		public void onReceive(Object msg) {
			...
			if (msg instanceof StorageApi.Entry && ((StorageApi.Entry) msg).key.equals(key)
					&& counter == null) {
				// Reply from Storage of the initial value, now we can create
				// the Counter
				final long value = ((StorageApi.Entry) msg).value;
				counter = getContext().actorOf(
						Props.create(Counter.class, key, value));
				...
				getContext()
						.system()
						.scheduler()
						.scheduleOnce(Duration.create(10, "seconds"),
								getSelf(), Reconnect,
								getContext().dispatcher(), null);
		...
		void forwardOrPlaceInBacklog(Object msg) {
			// We need the initial value from storage before we can start
			// delegate to
			// the counter. Before that we place the messages in a backlog, to
			// be sent
			// to the counter when it is initialized.
			if (counter == null) {
				if (backlog.size() >= MAX_BACKLOG)
					throw new CounterServiceApi.ServiceUnavailable(
							"CounterService not available,"
	...
	public static class Storage extends UntypedActor {
		...
		public void onReceive(Object msg) {
			...
			if (msg instanceof StorageApi.Store) {
				...
				getSender().tell(
						new StorageApi.Entry(get.key, value == null ? Long.valueOf(0L)
																	: value), getSelf());
			} else {
				unhandled(msg);
			}
	...
	public static class DummyDB {
		...
		public synchronized void save(String key, Long value)
				throws StorageApi.StorageException {
